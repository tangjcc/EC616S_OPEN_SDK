...
static const clock_ID_t g_uartClocks[USART_INSTANCE_NUM*2] = {GPR_UART0APBClk, GPR_UART0FuncClk,
                                                        GPR_UART1APBClk, GPR_UART1FuncClk,
                                                        GPR_UART2APBClk, GPR_UART2FuncClk
                                                        };

static const sw_reset_ID_t g_uartResetClocks[USART_INSTANCE_NUM*2] = {GPR_ResetUART0APB, GPR_ResetUART0Func,
...
#if USART_DEBUG
    ECOMM_TRACE(UNILOG_PLA_DRIVER, USART_IRQHandler_0, P_WARNING, 4, "instance: %d, iir_reg:0x%x, fcnr_reg:0x%x, rx_cnt:%d", instance, g_usartDataBase[instance].iir_reg, g_usartDataBase[instance].fcnr_reg, info->xfer.rx_cnt);
#endif
...
#if USART_DEBUG
                            ECOMM_TRACE(UNILOG_PLA_DRIVER, USART_IRQHandler_1, P_WARNING, 1, "dma transfer done, cnt:%d", current_cnt);
#endif
...
void USART_DmaRxEvent(uint32_t event, USART_RESOURCES *usart)
{
    ...
    switch (event)
    {
        case DMA_EVENT_END:
...
#if USART_DEBUG
            ECOMM_TRACE(UNILOG_PLA_DRIVER, USART_DmaRxEvent_0, P_SIG, 2, "uart dma rx event, fcnr:%x, cnt:%d", usart->reg->FCNR, dmaCurrentTargetAddress - (uint32_t)usart->info->xfer.rx_buf);
#endif
            ...
            {
#if USART_DEBUG
                ECOMM_TRACE(UNILOG_PLA_DRIVER, USART_DmaRxEvent_1, P_SIG, 0, "uart dma rx complete");
#endif
