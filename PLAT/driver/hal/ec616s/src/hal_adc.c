 ...
 * File name:    hal_adc.c
 * Description:  adc hal driver source file
 * History:      Rev1.0   2019-12-12
 *               Rev1.1   2020-02-29  Add api to get internal thermal temperature
 *               Rev1.2   2020-06-29  Align with EFUSE code location change
 *               Rev1.3   2020-07-17  Add conversion protection for reentry issue
 *
 ****************************************************************************/
...
/**
  \breif ADC EFUSE calibration code
 */
typedef struct _adc_efuse_cal_code
{
    uint32_t reserved    : 4;
    uint32_t code500 : 12;
    uint32_t code900 : 12;
} adc_efuse_cal_code_t;

/**
  \breif ADC EFUSE thermal code
 */
typedef struct _adc_efuse_thermal_code
{
    uint32_t code : 12;
    uint32_t reserved : 2;
    uint32_t offset : 10;
} adc_efuse_thermal_code_t;

typedef struct _adc_conversion_result
{
    volatile uint32_t rawCode;
    bool conversionOngoing;
    osSemaphoreId_t sem;
...
static volatile adc_efuse_cal_code_t g_adcEfuseCode = {0};
static volatile adc_efuse_thermal_code_t g_adcEfuseThermalCode = {0};
...
uint32_t HAL_ADC_CalibrateRawCode(uint32_t input)
{
    ...
    // Fetch calibration code from EFUSE for first use
    if(g_adcEfuseCode.code500 == 0)
    {
        EFuseSWReadSafePolling(10, 4, (uint8_t *)&g_adcEfuseCode);
    }

    // EFUSE value is invalid
    if(g_adcEfuseCode.code500 == 0)
    {
        return ((input * 1200) + 2048) / 4096;
    }
    else
    {
        temp = input - g_adcEfuseCode.code500;
        diff = g_adcEfuseCode.code900 - g_adcEfuseCode.code500;
        temp = (400 * temp + (diff >> 1)) / diff + 500;
...
/**
  ...
  Also, round the result in the way: round(a/b) = (a + b/2) / b
...
int32_t HAL_ADC_ConvertThermalRawCodeToTemperature(uint32_t input)
{
    ...
    // Fetch EFUSE thermal code for first use
    if(g_adcEfuseThermalCode.code == 0)
    {
        EFuseSWReadSafePolling(10, 4, (uint8_t *)&g_adcEfuseCode);
        EFuseSWReadSafePolling(14, 3, (uint8_t *)&g_adcEfuseThermalCode);

        // EFUSE value is valid
        if(g_adcEfuseCode.code900 != 0)
        {
            // calculate gain
            gain = -972970 / (int32_t)(g_adcEfuseCode.code900 - g_adcEfuseCode.code500);
        }
    }
    ...
    {
        return ((gain * (int32_t)(input - g_adcEfuseThermalCode.code) + (int32_t)(g_adcEfuseThermalCode.offset * 1000)) + 2000) / 4000;
    }
...
int32_t HAL_ADC_SampleVbatVoltage(uint32_t timeout_ms)
{
    ...
    if(g_adcVbatResult.conversionOngoing == true)
    {
        RestoreIRQMask(mask);
        return -256;
    }

    g_adcVbatResult.conversionOngoing = true;
    ...
    if(g_adcVbatResult.sem != NULL)
    {
        ...
        ADC_ChannelInit(ADC_ChannelVbat, ADC_UserAPP, &adcConfig, ADC_VbatCallback);
    ...
    mask = SaveAndSetIRQMask();
    g_adcVbatResult.conversionOngoing = false;
    RestoreIRQMask(mask);
...
int32_t HAL_ADC_GetThermalTemperature(uint32_t timeout_ms, int32_t* temperatruePtr)
{
    ...
    if(g_adcThermalResult.conversionOngoing == true)
    {
        RestoreIRQMask(mask);
        return -256;
    }

    g_adcThermalResult.conversionOngoing = true;
    ...
    if(g_adcThermalResult.sem != NULL)
    {
        ...
        ADC_ChannelInit(ADC_ChannelThermal, ADC_UserAPP, &adcConfig, ADC_ThermalCallback);
    ...
    mask = SaveAndSetIRQMask();
    g_adcThermalResult.conversionOngoing = false;
    RestoreIRQMask(mask);
