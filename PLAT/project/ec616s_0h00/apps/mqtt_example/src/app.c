...
static UINT8                    mqttEpSlpHandler = 0xff;
...
static void sendQueueMsg(UINT32 msgId, UINT32 xTickstoWait)
{
    ...
    if (psEventQueueHandle)
    {
        ...
        {
            ECOMM_TRACE(UNILOG_MQTT, mqttAppTask80, P_INFO, 0, "xQueueSend error");
        }
    }
}
...
static INT32 registerPSUrcCallback(urcID_t eventID, void *param, UINT32 paramLen)
{
    ...
    switch(eventID)
    {
        ...
        {
            rssi = *(UINT8 *)param;
            ECOMM_TRACE(UNILOG_MQTT, mqttAppTask81, P_INFO, 1, "RSSI signal=%d", rssi);
            break;
        }
        case NB_URC_ID_PS_BEARER_ACTED:
        {
            ECOMM_TRACE(UNILOG_MQTT, mqttAppTask82, P_INFO, 0, "Default bearer activated");
            break;
        }
        case NB_URC_ID_PS_BEARER_DEACTED:
        {
            ECOMM_TRACE(UNILOG_MQTT, mqttAppTask83, P_INFO, 0, "Default bearer Deactivated");
            break;
        }
        case NB_URC_ID_PS_CEREG_CHANGED:
        {
            ...
            gCellID = cereg->celId;
            ECOMM_TRACE(UNILOG_MQTT, mqttAppTask84, P_INFO, 4, "CEREG changed act:%d celId:%d locPresent:%d tac:%d", cereg->act, cereg->celId, cereg->locPresent, cereg->tac);
            break;
        }
...
static void mqttAppTask(void *arg)
{
    INT32 ret;
    ...
    {
        ECOMM_TRACE(UNILOG_MQTT, mqttAppTask0, P_INFO, 0, "psEventQueue create error!");
        return;
    }

    slpManApplyPlatVoteHandle("EP_MQTT",&mqttEpSlpHandler);
    slpManPlatVoteDisableSleep(mqttEpSlpHandler, SLP_SLP2_STATE);
    ECOMM_TRACE(UNILOG_MQTT, mqttAppTask1, P_INFO, 0, "first time run mqtt example");
        ...
        if (xQueueReceive(psEventQueueHandle, &queueItem, portMAX_DELAY))
        {
            switch(queueItem->messageId)
            {
                ...
                case QMSG_ID_NW_IPV4_6_READY:
                    ...
                    ECOMM_STRING(UNILOG_MQTT, mqttAppTask2, P_SIG, "IMSI = %s", gImsi);
                
                    appGetNetInfoSync(gCellID, &gNetworkInfo);
                    if ( NM_NET_TYPE_IPV4 == gNetworkInfo.body.netInfoRet.netifInfo.ipType)
                        ECOMM_TRACE(UNILOG_MQTT, mqttAppTask3, P_INFO, 4,"IP:\"%u.%u.%u.%u\"", ((UINT8 *)&gNetworkInfo.body.netInfoRet.netifInfo.ipv4Info.ipv4Addr.addr)[0],
                    ...
                    ECOMM_TRACE(UNILOG_MQTT, mqttAppTask4, P_INFO, 3, "tac=%d, cellID=%d ret=%d", tac, cellID, ret);
                    ...
                    ECOMM_TRACE(UNILOG_MQTT, mqttAppTask5, P_INFO, 4, "actType=%d, nwEdrxValueMs=%d nwPtwMs=%d ret=%d", actType, nwEdrxValueMs, nwPtwMs, ret);
...

                    {
                        ...
                        ECOMM_TRACE(UNILOG_MQTT, mqttAppTask6, P_INFO, 3, "Get PSM info mode=%d, TAU=%d, ActiveTime=%d", psmMode, tauTime, activeTime);
                    }
                    #ifdef EC_TEST_MQTT_ONENET
                    mqtt_demo_onenet();
                    #endif
                    #ifdef EC_TEST_MQTT_ALI
                    mqtt_demo_ali();
                    #endif                    
                    break;
...
static void appInit(void *arg)
{
    ...
    osThreadNew(mqttAppTask, NULL, &task_attr);
}
