...
static int lfs_bd_read(lfs_t *lfs,
    ...
    if (off+size > lfs->cfg->block_size) {
        ...
        //ECOMM_TRACE(UNILOG_LFS, lfs_bd_read_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
...
static int lfs_bd_flush(lfs_t *lfs,
    ...
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        ...
        if (validate) {
            ...
            if (res != LFS_CMP_EQ) {
#ifdef LFS_DEBUG_TRACE
                ECOMM_TRACE(UNILOG_LFS, lfs_bd_flush_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
                return LFS_ERR_CORRUPT;
            }
        }
...
static int lfs_dir_getread(lfs_t *lfs, const lfs_mdir_t *dir,
    ...
    if (off+size > lfs->cfg->block_size) {
#ifdef LFS_DEBUG_TRACE
        ECOMM_TRACE(UNILOG_LFS, lfs_dir_getread_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
        return LFS_ERR_CORRUPT;
    }
...
#ifdef LFS_DEBUG_TRACE
    ECOMM_TRACE(UNILOG_LFS, lfs_dir_fetchmatch_0, P_ERROR, 2, "Corrupted dir pair at %x %x", dir->pair[0], dir->pair[1]);
#endif
...
static int lfs_dir_commitcrc(lfs_t *lfs, struct lfs_commit *commit) {
    ...
    while (off < end) {
        uint32_t crc = LFS_BLOCK_NULL;
        for (lfs_off_t i = off; i < noff+sizeof(uint32_t); i += diff) {
            uint8_t dat[8];
            diff = lfs_min(noff+sizeof(uint32_t) - i, sizeof(dat));

            err = lfs_bd_read(lfs,
                    NULL, &lfs->rcache, noff+sizeof(uint32_t)-i,
                    commit->block, i, &dat, diff);
            if (err) {
                return err;
            }
        ...
        if (crc != 0) {
#ifdef LFS_DEBUG_TRACE
            ECOMM_TRACE(UNILOG_LFS, lfs_dir_commitcrc_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
            return LFS_ERR_CORRUPT;
        }
    ...
    while (true) {
        {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECOMM_TRACE(UNILOG_LFS, lfs_dir_compact_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECOMM_TRACE(UNILOG_LFS, lfs_dir_compact_1, P_WARNING, 0, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECOMM_TRACE(UNILOG_LFS, lfs_dir_compact_2, P_WARNING, 0, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (!lfs_pair_isnull(dir->tail)) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECOMM_TRACE(UNILOG_LFS, lfs_dir_compact_3, P_WARNING, 0, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
            ...
            if (!lfs_gstate_iszero(&delta)) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECOMM_TRACE(UNILOG_LFS, lfs_dir_compact_4, P_WARNING, 0, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECOMM_TRACE(UNILOG_LFS, lfs_dir_compact_5, P_WARNING, 0, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
    ...
    while (true) {
...

        {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECOMM_TRACE(UNILOG_LFS, lfs_ctz_extend_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (size != lfs->cfg->block_size) {
                for (lfs_off_t i = 0; i < size; i++) {
                    ...
                    if (err) {
                        if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                            ECOMM_TRACE(UNILOG_LFS, lfs_ctz_extend_1, P_WARNING, 0, "LFS corrupt detect!");
#endif
                            goto relocate;
                        }
                        return err;
            ...
            for (lfs_off_t i = 0; i < skips; i++) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECOMM_TRACE(UNILOG_LFS, lfs_ctz_extend_2, P_WARNING, 0, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
...
static int lfs_file_relocate(lfs_t *lfs, lfs_file_t *file) {
    ...
    while (true) {
        ...
        if (err) {
            if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                ECOMM_TRACE(UNILOG_LFS, lfs_file_relocate_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
                goto relocate;
            }
            return err;
        ...
        for (lfs_off_t i = 0; i < file->off; i++) {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECOMM_TRACE(UNILOG_LFS, lfs_file_relocate_1, P_WARNING, 0, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
        ...
        if (!(file->flags & LFS_F_INLINE)) {
            ...
            while (true) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECOMM_TRACE(UNILOG_LFS, lfs_file_flush_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
    ...
    while (nsize > 0) {
        ...
        while (true) {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECOMM_TRACE(UNILOG_LFS, lfs_file_write_0, P_WARNING, 0, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                file->flags |= LFS_F_ERRED;
