...
connection_t * ct_connection_find(connection_t * connList,
...
connection_t * ct_connection_new_incoming(connection_t * connList,
                                       int sock,
                                       struct sockaddr * addr,
                                       size_t addrLen)
{
    connection_t * connP;
    lwm2m_printf("ct_connection_new_incoming\n");
    connP = (connection_t *)ct_lwm2m_malloc(sizeof(connection_t));
    if (connP != NULL)
    {
        connP->sock = sock;
...
connection_t * ct_connection_create(connection_t * connList,
                                 int sock,
                                 char * host,
                                 char * port,
                                 int addressFamily)
...
int ct_connection_send(connection_t *connP,
                    uint8_t * buffer,
                    size_t length,uint8_t sendOption)
{
    int nbSent;
    size_t offset;
    lwm2m_printf("ct_connection_send:sendOption=%d\r\n",sendOption);
#ifdef WITH_LOGS
...
//    }
    ...
    lwm2m_printf("Sending %d bytes to [%s]:%hu\r\n", length, s, ntohs(port));
                          ...
                          void * userdata,uint8_t sendOption)
{
    ...
    if (-1 == ct_connection_send(connP, buffer, length,sendOption))
    {
