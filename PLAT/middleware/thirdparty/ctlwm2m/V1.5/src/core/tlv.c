...
int ct_lwm2m_decode_TLV(const uint8_t * buffer,
                    size_t buffer_len,
...
int ctlwm2m_tlv_parse(uint8_t * buffer,
              size_t bufferLen,
              lwm2m_data_t ** dataP)
{
    lwm2m_data_type_t type;
    ...
    *dataP = NULL;
    while (0 != (result = ct_lwm2m_decode_TLV((uint8_t*)buffer + index, bufferLen - index, &type, &id, &dataIndex, &dataLen)))
    {
        lwm2m_data_t * newTlvP;

        //ECOMM_TRACE(UNILOG_CTLWM2M, tlv_parse_1, P_INFO, 2, "ctlwm2m_tlv_parse len=%d type=%d", dataLen, type);

        newTlvP = ct_lwm2m_data_new(size + 1);
        if (size >= 1)
        {
            if (newTlvP == NULL)
            {
        ...
        *dataP = newTlvP;
        ...
        {
            (*dataP)[size].value.asChildren.count = ctlwm2m_tlv_parse(buffer + index + dataIndex,
                                                          dataLen,
                                                          &((*dataP)[size].value.asChildren.array));
            if ((*dataP)[size].value.asChildren.count == 0)
            {
                ct_lwm2m_data_free(size + 1, *dataP);
                return 0;
            }
        }
        else
        {
            //ECOMM_TRACE(UNILOG_CTLWM2M, tlv_parse_2, P_INFO, 0, "tlv_parse do ct_lwm2m_data_encode_opaque");
            ct_lwm2m_data_encode_opaque(buffer + index + dataIndex, dataLen, (*dataP) + size);
        }
        size++;
        index += result;
    }
...
int ctlwm2m_tlv_serialize(bool isResourceInstance, 
                  int size,
                  lwm2m_data_t * dataP,
                  uint8_t ** bufferP)
{
...
//    printf("tlv get len:%d\n",length);
    //ECOMM_TRACE(UNILOG_CTLWM2M, tlv_serialize_0, P_INFO, 1, "tlv get len:%d",length);
...
//        printf("tlv dataP[i].type:%d\n",dataP[i].type);
        //ECOMM_TRACE(UNILOG_CTLWM2M, tlv_serialize_1, P_INFO, 1, "tlv dataP[i].type:%d",dataP[i].type);
        ...
        case LWM2M_TYPE_OBJECT_INSTANCE:
            {
                ...
                res = ctlwm2m_tlv_serialize(isInstance, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, &tmpBuffer);
                if (res < 0)
                {
                    length = -1;
                }
