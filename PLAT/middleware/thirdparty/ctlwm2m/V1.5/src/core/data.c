                             ...
                             uint8_t ** bufferP)
{
    ...
    case LWM2M_TYPE_STRING:
        *bufferP = (uint8_t *)ct_lwm2m_malloc(dataP->value.asBuffer.length);
    ...
    case LWM2M_TYPE_INTEGER:
    {
        ...
        *bufferP = (uint8_t *)ct_lwm2m_malloc(res);
    ...
    case LWM2M_TYPE_BOOLEAN:
        *bufferP = (uint8_t *)ct_lwm2m_malloc(1);
        if (NULL == *bufferP) return -1;
...
static int prv_setBuffer(lwm2m_data_t * dataP,
                         ...
                         size_t bufferLen)
{
    //printf("prv_setBuffer:bufferLen:%d,buffer:%p\n",bufferLen,buffer);
    if(buffer!=NULL && bufferLen!=0)
    {
        dataP->value.asBuffer.buffer = (uint8_t *)ct_lwm2m_malloc(bufferLen);
        memset(dataP->value.asBuffer.buffer, 0, bufferLen);
    ...
    //printf("prv_setBuffer:dataP->value.asBuffer.length:%d\n",dataP->value.asBuffer.length);
    return 1;
}
...
lwm2m_data_t * ct_lwm2m_data_new(int size)
{
    ...
    dataP = (lwm2m_data_t *)ct_lwm2m_malloc(size * sizeof(lwm2m_data_t));
...
void ct_lwm2m_data_free(int size,
                     lwm2m_data_t * dataP)
{
    int i;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        case LWM2M_TYPE_OBJECT:
            ct_lwm2m_data_free(dataP[i].value.asChildren.count, dataP[i].value.asChildren.array);
            break;
        ...
        case LWM2M_TYPE_OPAQUE:
            ...
            {
                ct_lwm2m_free(dataP[i].value.asBuffer.buffer);
            }
...
void ct_lwm2m_data_encode_string(const char * string,
                              lwm2m_data_t * dataP)
{
    size_t len;
    int res;
...
void ct_lwm2m_data_encode_opaque(uint8_t * buffer,
                              size_t length,
                              lwm2m_data_t * dataP)
{
    int res;
...
void lwm2m_data_decode_opaque(const lwm2m_data_t * dataP,unsigned short* msgID,char** data)
{
	...
	*msgID = (dataP->value.asBuffer.buffer)[j] << 8 | (dataP->value.asBuffer.buffer)[j+1];
	j += 2;
	*data = ct_lwm2m_malloc(dataP->value.asBuffer.length);
	for(i = 0; i < dataP->value.asBuffer.length-2; i++)
	{
		(*data)[i] = (dataP->value.asBuffer.buffer)[j];
	}
...
uint8_t* ctiot_funcv1_coap_decode_message(uint8_t* payload,uint32_t payloadLen,uint16_t* msgId)
{
	...
	*msgId=0;
	if(payload !=NULL && payloadLen >=2)
	{
		...
		*msgId =(uint16_t)payload[0] << 8 | payload[1];
		if(realPayloadLen > 0)
		{
			realPayload = ct_lwm2m_malloc(realPayloadLen);
			if(realPayload != NULL)
			{
...
void ct_lwm2m_data_encode_nstring(const char * string,
                               size_t length,
                               lwm2m_data_t * dataP)
{
    LOG_ARG("length: %d, string: \"%s\"", length, string);
    //log_print_plain_text("length: %d, string: \"%s\"\n", length, string);
    ct_lwm2m_data_encode_opaque((uint8_t *)string, length, dataP);
...
void ct_lwm2m_data_encode_int(int64_t value,
                           lwm2m_data_t * dataP)
{
...
int ct_lwm2m_data_decode_int(const lwm2m_data_t * dataP,
                          int64_t * valueP)
{
    int result;
...
void ct_lwm2m_data_encode_float(double value,
                             lwm2m_data_t * dataP)
{
...
int ct_lwm2m_data_decode_float(const lwm2m_data_t * dataP,
                            double * valueP)
{
    int result;
...
void ct_lwm2m_data_encode_bool(bool value,
                            lwm2m_data_t * dataP)
{
...
int ct_lwm2m_data_decode_bool(const lwm2m_data_t * dataP,
                           bool * valueP)
{
    int result;
...
void ct_lwm2m_data_encode_objlink(uint16_t objectId,
...
void ct_lwm2m_data_include(lwm2m_data_t * subDataP,
                        size_t count,
                        lwm2m_data_t * dataP)
{
...
void ct_lwm2m_data_encode_instances(lwm2m_data_t * subDataP,
                                 size_t count,
                                 lwm2m_data_t * dataP)
{
    LOG_ARG("count: %d", count);
    ct_lwm2m_data_include(subDataP, count, dataP);
    dataP->type = LWM2M_TYPE_MULTIPLE_RESOURCE;
}

int ct_lwm2m_data_parse(lwm2m_uri_t * uriP,
                     uint8_t * buffer,
                     size_t bufferLen,
                     ...
                     lwm2m_data_t ** dataP)
{
    ...
    LOG_URI(uriP);
    ECOMM_TRACE(UNILOG_CTLWM2M, lwm2m_data_parse_0, P_INFO, 2, "ct_lwm2m_data_parse format=%d bufferlen=%d",format, bufferLen);
    switch (format)
    {
    ...
    case LWM2M_CONTENT_TLV:
        return ctlwm2m_tlv_parse(buffer, bufferLen, dataP);
#ifdef LWM2M_SUPPORT_JSON
#ifdef LWM2M_OLD_CONTENT_FORMAT_SUPPORT
    case LWM2M_CONTENT_JSON_OLD:
#endif
    case LWM2M_CONTENT_JSON:
        return ct_json_parse(uriP, buffer, bufferLen, dataP);
#endif
...
int ct_lwm2m_data_serialize(lwm2m_uri_t * uriP,
                         int size,
                         lwm2m_data_t * dataP,
                         ...
                         uint8_t ** bufferP)
{
    ...
    //ECOMM_TRACE(UNILOG_CTLWM2M, lwm2m_data_serialize_0, P_INFO, 1, "ct_lwm2m_data_serialize format: %d",*formatP);
    switch (*formatP)
    {
    case LWM2M_CONTENT_TEXT:
    ...
    case LWM2M_CONTENT_OPAQUE:
        *bufferP = (uint8_t *)ct_lwm2m_malloc(dataP->value.asBuffer.length);
        if (*bufferP == NULL) return -1;
        memcpy(*bufferP, dataP->value.asBuffer.buffer, dataP->value.asBuffer.length);
        //printf("ct_lwm2m_data_serialize:%lu\n",dataP->value.asBuffer.length);
        return (int)dataP->value.asBuffer.length;
    ...
    case LWM2M_CONTENT_TLV_OLD:
    {
            ...
            return ctlwm2m_tlv_serialize(isResourceInstance, size, dataP, bufferP);
        }

#ifdef LWM2M_CLIENT_MODE
    case LWM2M_CONTENT_LINK:
        return ct_discover_serialize(NULL, uriP, NULL, size, dataP, bufferP);
#endif
#ifdef LWM2M_SUPPORT_JSON
    case LWM2M_CONTENT_JSON:
    case LWM2M_CONTENT_JSON_OLD:
        return ct_json_serialize(uriP, size, dataP, bufferP);
#endif
