                         ...
                         _record_t * recordP)
{
    ...
    do
    {
        ...
        case 2:
        {
            ...
            case 'b':
                if (recordP->type != _TYPE_UNSET) return -1;
                if (0 == ct_lwm2m_strncmp(JSON_TRUE_STRING, (char *)buffer + index + valueStart, valueLen))
                {
                    recordP->type = _TYPE_TRUE;
                }
                else if (0 == ct_lwm2m_strncmp(JSON_FALSE_STRING, (char *)buffer + index + valueStart, valueLen))
                {
                             ...
                             lwm2m_data_t * targetP)
{
    ...
    {
    case _TYPE_FALSE:
        ct_lwm2m_data_encode_bool(false, targetP);
        break;
    case _TYPE_TRUE:
        ct_lwm2m_data_encode_bool(true, targetP);
        break;
    case _TYPE_FLOAT:
    {
        size_t i;
        ...
        if (i == recordP->valueLen)
        {
            ...
            ct_lwm2m_data_encode_int(value, targetP);
        }
        else
        {
            double value;
            ...
            ct_lwm2m_data_encode_float(value, targetP);
        }
    }
    break;

    case _TYPE_STRING:
        ct_lwm2m_data_encode_opaque(recordP->value, recordP->valueLen, targetP);
...
static lwm2m_data_t * prv_extendData(lwm2m_data_t * parentP)
{
    ...
    newP = ct_lwm2m_data_new(parentP->value.asChildren.count + 1);
    ...
    if (parentP->value.asChildren.array != NULL)
    {
        ...
        ct_lwm2m_free(parentP->value.asChildren.array);     // do not use ct_lwm2m_data_free() to keep pointed values
    }
                             ...
                             lwm2m_data_t ** dataP)
{
    ...
    else
    {
        ...
        *dataP = ct_lwm2m_data_new(1);
        ...
        {
            parentP->value.asChildren.count = 1;
            parentP->value.asChildren.array = ct_lwm2m_data_new(1);
        ...
        parentP->value.asChildren.array = ct_lwm2m_data_new(count);
        if (NULL == parentP->value.asChildren.array) goto error;
                         ...
                         lwm2m_data_t ** resultP)
{
    ...
    *resultP = ct_lwm2m_data_new(realSize);
...
int ct_json_parse(lwm2m_uri_t * uriP,
               uint8_t * buffer,
               size_t bufferLen,
               lwm2m_data_t ** dataP)
{
    ...
    *dataP = NULL;
    ...
    do
    {
        ...
        case 'e':
        {
            ...
            recordArray = (_record_t*)ct_lwm2m_malloc(count * sizeof(_record_t));
    ...
    if (eFound == true)
    {
        ...
        else
        {
            ...
            {
                res = ctlwm2m_stringToUri((char *)buffer + bnStart, bnLen, &baseURI);
                    ...
                    for (i = 0 ; i < size && resP == NULL; i++)
                    {
                        ...
                        if (targetP->id == uriP->resourceId)
                        {
                            ...
                            else
                            {
                                ...
                                ct_lwm2m_data_free(count, parsedP);
                                parsedP = NULL;
                            }
                        }
                    }
                              ...
                              size_t bufferLen)
{
    ...
    case LWM2M_TYPE_INTEGER:
    {
        ...
        if (0 == ct_lwm2m_data_decode_int(tlvP, &value)) return -1;
    ...
    case LWM2M_TYPE_FLOAT:
    {
        ...
        if (0 == ct_lwm2m_data_decode_float(tlvP, &value)) return -1;
    ...
    case LWM2M_TYPE_BOOLEAN:
    {
        ...
        if (0 == ct_lwm2m_data_decode_bool(tlvP, &value)) return -1;
...
int ct_prv_serializeData(lwm2m_data_t * tlvP,
                      ...
                      size_t bufferLen)
{
    ...
    case LWM2M_TYPE_MULTIPLE_RESOURCE:
    {
        ...
        {
            res = ct_prv_serializeData(tlvP->value.asChildren.array + index, uriStr, uriLen, buffer + head, bufferLen - head);
            if (res < 0) return -1;
            head += res;
        }
    }
...
int ct_json_serialize(lwm2m_uri_t * uriP,
                   int size,
                   lwm2m_data_t * tlvP,
                   uint8_t ** bufferP)
{
    ...
    baseUriLen = ctlwm2m_uri_toString(uriP, baseUriStr, URI_MAX_STRING_LEN, &rootLevel);
    ...
    for (index = 0 ; index < num && head < PRV_JSON_BUFFER_SIZE ; index++)
    {
        ...
        res = ct_prv_serializeData(targetP + index, NULL, 0, bufferJSON + head, PRV_JSON_BUFFER_SIZE - head);
        if (res < 0) return 0;
        head += res;
    }
    ...
    *bufferP = (uint8_t *)ct_lwm2m_malloc(head);
    if (*bufferP == NULL) 
		return 0;
	
    memcpy(*bufferP, bufferJSON, head);
	
    lwm2m_printf("json buffer:%s\n",*bufferP);
