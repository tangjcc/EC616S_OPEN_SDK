                             ...
                             conn_m_data_t * connDataP)
{
    ...
    case RES_M_NETWORK_BEARER:
        ct_lwm2m_data_encode_int(VALUE_NETWORK_BEARER_NBIoT, dataP);
    ...
    case RES_M_AVL_NETWORK_BEARER:
    {
        ...
        subTlvP = ct_lwm2m_data_new(riCnt);
        subTlvP[0].id    = 0;
        ct_lwm2m_data_encode_int(VALUE_AVL_NETWORK_BEARER_1, subTlvP);
        ct_lwm2m_data_encode_instances(subTlvP, riCnt, dataP);
        return COAP_205_CONTENT ;
    }

    case RES_M_RADIO_SIGNAL_STRENGTH: //s-int
    {
        uint32_t radio_signal_strenght = ct_conn_radiosignalstrength();
        ct_lwm2m_data_encode_int(radio_signal_strenght, dataP);
        return COAP_205_CONTENT;
    }
    /*
    case RES_O_LINK_QUALITY: //s-int
        ct_lwm2m_data_encode_int(connDataP->linkQuality, dataP);
        return COAP_205_CONTENT ;
        */
    ...
    case RES_M_IP_ADDRESSES:
    {
        ...
        lwm2m_data_t* subTlvP = ct_lwm2m_data_new(riCnt);
        for (ri = 0; ri < riCnt; ri++)
        {
            subTlvP[ri].id = ri;
            ct_conn_localIP(connDataP->ipAddresses[ri]);
            ct_lwm2m_data_encode_string(connDataP->ipAddresses[ri], subTlvP + ri);
        }
        ct_lwm2m_data_encode_instances(subTlvP, riCnt, dataP);
        return COAP_205_CONTENT ;
    }
    ...
    case RES_O_ROUTER_IP_ADDRESS:
    {
        ...
        {
            subTlvP[ri].id = ri;
            ct_lwm2m_data_encode_string(connDataP->routerIpAddresses[ri], subTlvP + ri);
        }
        ct_lwm2m_data_encode_instances(subTlvP, riCnt, dataP);
        return COAP_205_CONTENT ;
    }
    ...
    case RES_O_LINK_UTILIZATION:
        ct_lwm2m_data_encode_int(connDataP->linkUtilization, dataP);
        return COAP_205_CONTENT;
    ...
    case RES_O_APN:
    {
        ...
        subTlvP = ct_lwm2m_data_new(riCnt);
        subTlvP[0].id     = 0;
        ct_lwm2m_data_encode_string(VALUE_APN_1, subTlvP);
        ct_lwm2m_data_encode_instances(subTlvP, riCnt, dataP);
        return COAP_205_CONTENT;
    }
        break;
    ...
    case RES_O_CELL_ID:
    {
        uint32_t cell_id = ct_conn_cellid();
        ct_lwm2m_data_encode_int(cell_id, dataP);
        return COAP_205_CONTENT ;
    }
    /*
    case RES_O_SMNC:
        ct_lwm2m_data_encode_int(VALUE_SMNC, dataP);
        return COAP_205_CONTENT ;

    case RES_O_SMCC:
        ct_lwm2m_data_encode_int(VALUE_SMCC, dataP);
        return COAP_205_CONTENT ;
        */
...
{
    /*
     * The ct_get_object_conn_m() function create the object itself and return a pointer to the structure that represent it.
     */
    lwm2m_object_t * connObj;

    connObj = (lwm2m_object_t *) ct_lwm2m_malloc(sizeof(lwm2m_object_t));
    ...
    if (NULL != connObj)
    {
         ...
         *
         */
        connObj->instanceList = (lwm2m_list_t *)ct_lwm2m_malloc(sizeof(lwm2m_list_t));
        ...
        connObj->userData = ct_lwm2m_malloc(sizeof(conn_m_data_t));
...
uint8_t ct_connectivity_moni_change(lwm2m_data_t * dataArray,
                                 lwm2m_object_t * objectP)
{
    int64_t value;
    uint8_t result;
