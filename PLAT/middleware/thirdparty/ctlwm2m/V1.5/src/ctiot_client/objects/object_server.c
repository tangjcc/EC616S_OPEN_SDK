...
typedef struct _server_instance_
{
    ...
    bool        storing;
    char        binding[4];
} ct_server_instance_t;

//int updateRegistration(lwm2m_context_t * contextP,lwm2m_server_t * server);
static uint8_t ct_prv_get_value(lwm2m_data_t * dataP,
                             ct_server_instance_t * targetP)
{
    switch (dataP->id)
    {
    case LWM2M_SERVER_SHORT_ID_ID:
        ct_lwm2m_data_encode_int(targetP->shortServerId, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SERVER_LIFETIME_ID:
        ct_lwm2m_data_encode_int(targetP->lifetime, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SERVER_MIN_PERIOD_ID:
        ct_lwm2m_data_encode_int(targetP->defaultMinPeriod, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SERVER_MAX_PERIOD_ID:
        ct_lwm2m_data_encode_int(targetP->defaultMaxPeriod, dataP);
        return COAP_205_CONTENT;
/*
    ...
    case LWM2M_SERVER_TIMEOUT_ID:
        ct_lwm2m_data_encode_int(targetP->disableTimeout, dataP);
        return COAP_205_CONTENT;
*/
    case LWM2M_SERVER_STORING_ID:
        ct_lwm2m_data_encode_bool(targetP->storing, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SERVER_BINDING_ID:
        ct_lwm2m_data_encode_string(targetP->binding, dataP);
        return COAP_205_CONTENT;
/*
...
static uint8_t ct_prv_server_read(uint16_t instanceId,
                               int * numDataP,
                               ...
                               lwm2m_object_t * objectP)
{
    ...
    targetP = (ct_server_instance_t *)lwm2m_list_find(objectP->instanceList, instanceId);
...
static uint8_t ct_prv_server_discover(uint16_t instanceId,
                                   int * numDataP,
                                   lwm2m_data_t ** dataArrayP,
                                   lwm2m_object_t * objectP)
{
...
static uint8_t ct_prv_set_int_value(lwm2m_data_t * dataArray, uint32_t * data) {
    uint8_t result;
    int64_t value;
...
static uint8_t ct_prv_server_write(uint16_t instanceId,
                                int numData,
                                ...
                                lwm2m_object_t * objectP)
{
    ...
    do
    {
        ...
        case LWM2M_SERVER_SHORT_ID_ID:
            {
                ...
                result = ct_prv_set_int_value(dataArray + i, &value);
        ...
        case LWM2M_SERVER_LIFETIME_ID:
        {
            ...
            {
                if (value >= 300 && value <= 0xFFFFFFFF)
                {
                    targetP->lifetime = value;
                    ctiot_funcv1_context_t* pContext=ctiot_funcv1_get_context();
                    pContext->lifetime = value;
                    c2f_encode_params(pContext);
                    //updateRegistration(ctiot_funcv1_get_context(),lwm2m_server_t * server)
                    result = COAP_204_CHANGED;
                }
                else
                {
            ...
            //result = ct_prv_set_int_value(dataArray + i, (uint32_t *)&(targetP->lifetime));
            break;
        }
        case LWM2M_SERVER_MIN_PERIOD_ID:
            result = ct_prv_set_int_value(dataArray + i, &(targetP->defaultMinPeriod));
            break;

        case LWM2M_SERVER_MAX_PERIOD_ID:
            result = ct_prv_set_int_value(dataArray + i, &(targetP->defaultMaxPeriod));
            break;
/*
        case LWM2M_SERVER_DISABLE_ID:
        ...
        case LWM2M_SERVER_TIMEOUT_ID:
            result = ct_prv_set_int_value(dataArray + i, &(targetP->disableTimeout));
            break;
            */
        ...
        case LWM2M_SERVER_STORING_ID:
        {
            ...
            if (1 == ct_lwm2m_data_decode_bool(dataArray + i, &value))
            {
...
static uint8_t ct_prv_server_execute(uint16_t instanceId,
                                  uint16_t resourceId,
                                  uint8_t * buffer,
                                  int length,
                                  lwm2m_object_t * objectP)
...
static uint8_t ct_prv_server_delete(uint16_t id,
                                 lwm2m_object_t * objectP)
{
    ct_server_instance_t * serverInstance;
    ...
    ct_lwm2m_free(serverInstance);

    return COAP_202_DELETED;
    
}

static uint8_t ct_prv_server_create(uint16_t instanceId,
                                 int numData,
                                 lwm2m_data_t * dataArray,
                                 lwm2m_object_t * objectP)
{
    ...
    serverInstance = (ct_server_instance_t *)ct_lwm2m_malloc(sizeof(ct_server_instance_t));
    if (NULL == serverInstance) return COAP_500_INTERNAL_SERVER_ERROR;
    memset(serverInstance, 0, sizeof(ct_server_instance_t));

    serverInstance->instanceId = instanceId;
    objectP->instanceList = LWM2M_LIST_ADD(objectP->instanceList, serverInstance);

    result = ct_prv_server_write(instanceId, numData, dataArray, objectP);

    if (result != COAP_204_CHANGED)
    {
        (void)ct_prv_server_delete(instanceId, objectP);
    }
    else
    {
...
void ct_copy_server_object(lwm2m_object_t * objectDest, lwm2m_object_t * objectSrc)
{
    memcpy(objectDest, objectSrc, sizeof(lwm2m_object_t));
    objectDest->instanceList = NULL;
    objectDest->userData = NULL;
    ct_server_instance_t * instanceSrc = (ct_server_instance_t *)objectSrc->instanceList;
    ct_server_instance_t * previousInstanceDest = NULL;
    while (instanceSrc != NULL)
    {
        ct_server_instance_t * instanceDest = (ct_server_instance_t *)ct_lwm2m_malloc(sizeof(ct_server_instance_t));
        if (NULL == instanceDest)
        {
            return;
        }
        memcpy(instanceDest, instanceSrc, sizeof(ct_server_instance_t));
        // not sure it's necessary:
        strcpy(instanceDest->binding, instanceSrc->binding);
        instanceSrc = (ct_server_instance_t *)instanceSrc->next;
...
void ct_display_server_object(lwm2m_object_t * object)
{
#ifdef WITH_LOGS
    lwm2m_printf("  /%u: Server object, instances:\r\n", object->objID);
    ct_server_instance_t * serverInstance = (ct_server_instance_t *)object->instanceList;
    while (serverInstance != NULL)
    {
        ...
        serverInstance = (ct_server_instance_t *)serverInstance->next;
    }
#endif
}

lwm2m_object_t * ct_get_server_object(int serverId,
                                   const char* binding,
                                   int lifetime,
                                   bool storing)
{
    ...
    serverObj = (lwm2m_object_t *)ct_lwm2m_malloc(sizeof(lwm2m_object_t));
    ...
    if (NULL != serverObj)
    {
        ...
        serverObj->readFunc = ct_prv_server_read;
        serverObj->discoverFunc = ct_prv_server_discover;
        serverObj->writeFunc = ct_prv_server_write;
        serverObj->createFunc = ct_prv_server_create;
        serverObj->deleteFunc = ct_prv_server_delete;
        serverObj->executeFunc = ct_prv_server_execute;
    }
