...
static uint8_t prv_set_tlv(lwm2m_data_t * dataP, conn_s_data_t * connStDataP)
{
    ...
    case RES_O_SMS_TX_COUNTER:
        ct_lwm2m_data_encode_int(connStDataP->smsTxCounter, dataP);
        return COAP_205_CONTENT;
    case RES_O_SMS_RX_COUNTER:
        ct_lwm2m_data_encode_int(connStDataP->smsRxCounter, dataP);
        return COAP_205_CONTENT;
    case RES_O_TX_DATA:
        ct_lwm2m_data_encode_int(connStDataP->txDataByte/1024, dataP);
        return COAP_205_CONTENT;
    case RES_O_RX_DATA:
        ct_lwm2m_data_encode_int(connStDataP->rxDataByte/1024, dataP);
        return COAP_205_CONTENT;
    case RES_O_MAX_MESSAGE_SIZE:
        ct_lwm2m_data_encode_int(connStDataP->maxMessageSize, dataP);
        return COAP_205_CONTENT;
    case RES_O_AVERAGE_MESSAGE_SIZE:
        ct_lwm2m_data_encode_int(connStDataP->avrMessageSize, dataP);
        return COAP_205_CONTENT;
    default:
...
void ct_conn_s_updateTxStatistic(lwm2m_object_t * objectP, uint16_t txDataByte, bool smsBased)
{
    conn_s_data_t* myData = (conn_s_data_t*) (objectP->userData);
...
void ct_conn_s_updateRxStatistic(lwm2m_object_t * objectP, uint16_t rxDataByte, bool smsBased)
{
	if( objectP->objID != LWM2M_CONN_STATS_OBJECT_ID )
	{
		lwm2m_printf("input object err, please check object!\r\n");
		return;
	}
...
lwm2m_object_t * ct_get_object_conn_s(void)
{
    /*
     * The ct_get_object_conn_s() function create the object itself and return
    ...
    if (NULL != connObj)
    {
        ...
        connObj->instanceList = ct_lwm2m_malloc(sizeof(lwm2m_list_t));
        ...
        connObj->userData     = ct_lwm2m_malloc(sizeof(conn_s_data_t));
