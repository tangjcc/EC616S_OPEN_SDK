                                 ...
                                 lwm2m_server_t * bootstrapServer)
{
    ...
    {
        //EC add begin
        if (context->connectServerCallback) {
            bootstrapServer->sessionH = context->connectServerCallback(bootstrapServer->secObjInstID, context->userData);
        } else {
...
uint8_t lwm2m_bootstrap_handleFinish(lwm2m_context_t * context,
                               void * fromSessionH)
{
    lwm2m_server_t * bootstrapServer;
    ...
    while (targetP != NULL)
    {
        ...
        if (targetP->sessionH == NULL)
        {
            ...
            if (contextP->connectServerCallback) {
                targetP->sessionH = contextP->connectServerCallback(targetP->secObjInstID, contextP->userData);
            } else {
...
uint8_t lwm2m_bootstrap_handleCommand(lwm2m_context_t * contextP,
                                lwm2m_uri_t * uriP,
                                lwm2m_server_t * serverP,
                                coap_packet_t * message,
