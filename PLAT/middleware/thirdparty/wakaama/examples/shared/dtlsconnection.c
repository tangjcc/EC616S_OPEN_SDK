        ...
        dataP->type == LWM2M_TYPE_OPAQUE)
    {
        ...
        if (buff != 0)
        {
            ...
            *length = dataP->value.asBuffer.length;
            ECOMM_DUMP(UNILOG_LWM2M, security_get_public_id_2, P_INFO, "psk id:",*length,(uint8_t*)buff);
        }
        lwm2m_data_free(size, dataP);
...
char * security_get_secret_key(lwm2m_object_t * obj, int instanceId, int * length){
        ...
        dataP->type == LWM2M_TYPE_OPAQUE)
    {
        ...
        if (buff != 0)
        {
            memcpy(buff, dataP->value.asBuffer.buffer, dataP->value.asBuffer.length);
            *length = dataP->value.asBuffer.length;
            ECOMM_DUMP(UNILOG_LWM2M, security_get_secret_key_2, P_INFO, "psk:",*length,(uint8_t*)buff);
        }
        lwm2m_data_free(size, dataP);
    ...
    if (s >= 0)
    {
        ...
        if (connP != NULL)
        {
                     ...
                     != LWM2M_SECURITY_MODE_NONE)
            {
                ...
                ECOMM_TRACE(UNILOG_LWM2M, dtls_connection_create_2, P_INFO, 1, "new dtlscontext, it's peer=%x",connP->dtlsContext->peers);
            }
            else
            {
                // no dtls session
...
int connection_send(dtls_connection_t *connP, uint8_t * buffer, size_t length){
    if (connP->dtlsSession == NULL) {
        ...
        ECOMM_TRACE(UNILOG_LWM2M, dtlsconnection_send_4, P_INFO, 2, "dtlsSession = %x,peer = %x,call dtls_write",connP->dtlsSession,connP->dtlsContext->peers);
