...
int mbedtls_ssl_fetch_input( mbedtls_ssl_context *ssl, size_t nb_want )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_fetch_input_1, P_INFO, 0, "=> fetch input");
...
#endif
    {
        MBEDTLS_SSL_DEBUG_MSG( 2, ( "in_left: %d, nb_want: %d",
                       ssl->in_left, nb_want ) );
        //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_fetch_input_2, P_INFO, 2, "in_left: %d, nb_want: %d",
        //               ssl->in_left, nb_want );
        ...
        while( ssl->in_left < nb_want )
        {
            ...
            MBEDTLS_SSL_DEBUG_MSG( 2, ( "in_left: %d, nb_want: %d",
                                        ssl->in_left, nb_want ) );
            MBEDTLS_SSL_DEBUG_RET( 2, "ssl->f_recv(_timeout)", ret );
            //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_fetch_input_3, P_INFO, 3, "in_left: %d, nb_want: %d, ssl->f_recv ret:0x%x",
            //               ssl->in_left, nb_want, ret);
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= fetch input" ) );
    //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_fetch_input_4, P_INFO, 0, "<= flush input");
...
int mbedtls_ssl_flush_output( mbedtls_ssl_context *ssl )
{
    ...
    //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_flush_output_1, P_INFO, 0, "=> flush output");
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= flush output" ) );
        //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_flush_output_2, P_INFO, 0, "<= flush output.out_left = 0");
        return( 0 );
    }
    ...
    while( ssl->out_left > 0 )
    {
        ...
        //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_flush_output_3, P_INFO, 2, "message length: %d, out_left: %d",mbedtls_ssl_hdr_len( ssl ) + ssl->out_msglen, ssl->out_left );

        buf = ssl->out_hdr + mbedtls_ssl_hdr_len( ssl ) +
              ssl->out_msglen - ssl->out_left;
        ret = ssl->f_send( ssl->p_bio, buf, ssl->out_left, ssl->rai );

        MBEDTLS_SSL_DEBUG_RET( 2, "ssl->f_send", ret );
        ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_flush_output_4, P_INFO, 1, "ssl->f_send return:0x%x",ret);
    ...
    if( i == ssl_ep_len( ssl ) )
    {
        ...
        //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_flush_output_5, P_INFO, 0, "outgoing message counter would wrap");
        return( MBEDTLS_ERR_SSL_COUNTER_WRAPPING );
    }

    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= flush output" ) );
    //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_flush_output_6, P_INFO, 0, "<= flush output");
...
int mbedtls_ssl_resend( mbedtls_ssl_context *ssl )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_resend_1, P_ERROR, 1, "=> mbedtls_ssl_resend (retransmit_state=%d)",ssl->handshake->retransmit_state);
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= mbedtls_ssl_resend" ) );
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_resend_2, P_ERROR, 1, "<= mbedtls_ssl_resend (retransmit_state=%d)",ssl->handshake->retransmit_state);
...
int mbedtls_ssl_write_record( mbedtls_ssl_context *ssl )
{
    ...
    //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_write_record_1, P_WARNING, 0, "=> write record");
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= write record" ) );
    //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_write_record_2, P_WARNING, 0, "<= write record");
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
            ...
            recv_msg_seq != ssl->handshake->in_msg_seq )
        {
                ...
                ssl->in_msg[0] != MBEDTLS_SSL_HS_HELLO_VERIFY_REQUEST )
            {
                ...
                ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_prepare_handshake_record, P_INFO, 0, "received message from last flight,");
...
static int ssl_handle_possible_reconnect( mbedtls_ssl_context *ssl )
{
    ...
    if( ret == MBEDTLS_ERR_SSL_HELLO_VERIFY_REQUIRED )
    {
        ...
        (void) ssl->f_send( ssl->p_bio, ssl->out_buf, len, 0 );
...
static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
                         ...
                         - (size_t)( ssl->in_msg - ssl->in_buf ) )
    {
        ...
        int maxBufLen = MBEDTLS_SSL_BUFFER_LEN- (size_t)( ssl->in_msg - ssl->in_buf );
        ECOMM_TRACE(UNILOG_SSL, ssl_parse_record_header_1, P_INFO, 2, "ssl->in_msglen=%d maxBufLen = %d",ssl->in_msglen,maxBufLen);
        return( MBEDTLS_ERR_SSL_INVALID_RECORD );
    }
    ...
    if( ssl->transform_in == NULL )
    {
        ...
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            int maxContentLen = MBEDTLS_SSL_MAX_CONTENT_LEN;
            ECOMM_TRACE(UNILOG_SSL, ssl_parse_record_header_2, P_INFO, 2, "ssl->in_msglen=%d MBEDTLS_SSL_MAX_CONTENT_LEN = %d",ssl->in_msglen,maxContentLen);
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
    }
    else
...
static int ssl_prepare_record_content( mbedtls_ssl_context *ssl )
{
    ...
    if( !done && ssl->transform_in != NULL )
    {
        ...
        if( ssl->in_msglen > MBEDTLS_SSL_MAX_CONTENT_LEN )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            int maxContentLen = MBEDTLS_SSL_MAX_CONTENT_LEN;
            ECOMM_TRACE(UNILOG_SSL, ssl_prepare_record_content_1, P_INFO, 2, "ssl->in_msglen=%d MBEDTLS_SSL_MAX_CONTENT_LEN = %d",ssl->in_msglen,maxContentLen);
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
    }
...
int mbedtls_ssl_read_record( mbedtls_ssl_context *ssl )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_read_record_1, P_INFO, 0, "=> read record");
    ...
    if( ssl->keep_current_message == 0 )
    {
        do {
            ...
            {
                ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_read_record_2, P_INFO, 1, "ret = -0x%x",-ret);
...
{
    int ret;
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_read_record_layer_1, P_INFO, 0, "enter");
    ...
    if( ssl->in_hslen != 0 )
    {
        //ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_read_record_layer_2, P_INFO, 0, "Handshake messages");
...
read_record_header:
	...
	ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_read_record_layer_2, P_INFO, 0, "call mbedtls_ssl_fetch_input");

    if( ( ret = mbedtls_ssl_fetch_input( ssl, mbedtls_ssl_hdr_len( ssl ) ) ) != 0 )
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ssl_fetch_input", ret );
		ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_read_record_layer_3, P_INFO, 1, "ret=-0x%x",-ret);
        return( ret );
    }
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_write_certificate_1, P_INFO, 0, ".....=> mbedtls_ssl_write_certificate");
...
int mbedtls_ssl_parse_certificate( mbedtls_ssl_context *ssl )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_parse_certificate_01, P_WARNING, 0, ".....=> mbedtls_ssl_parse_certificate");
...
int mbedtls_ssl_parse_certificate( mbedtls_ssl_context *ssl )
{
    ...
    uint8_t alert;
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_parse_certificate_1, P_INFO, 0, "....=> mbedtls_ssl_parse_certificate");
...
int mbedtls_ssl_write_change_cipher_spec( mbedtls_ssl_context *ssl )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_write_change_cipher_spec_1, P_INFO, 0, ".....=> mbedtls_ssl_write_change_cipher_spec");
...
int mbedtls_ssl_write_finished( mbedtls_ssl_context *ssl )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_write_finished_1, P_INFO, 0, ".....=> mbedtls_ssl_write_finished");
                       ...
                       const mbedtls_ssl_config *conf )
{
	...
	ssl->in_buf = mbedtls_calloc( 1, len );	
...
#endif
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_handshake_step, P_INFO, 1, "mbedtls_ssl_handshake_client_step return -0x%x", -ret);
...
int mbedtls_ssl_handshake( mbedtls_ssl_context *ssl )
{
    ...
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_handshake_1, P_WARNING, 0, "=> handshake");
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= handshake" ) );
    ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_handshake_2, P_WARNING, 0, "<= handshake");
...
int mbedtls_ssl_read( mbedtls_ssl_context *ssl, unsigned char *buf, size_t len )
{
    ...
    if( ssl->in_offt == NULL )
    {
		...
		ECOMM_TRACE(UNILOG_SSL, mbedtls_ssl_read_1, P_INFO, 0, "call mbedtls_ssl_read_record");
        if( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
        {
