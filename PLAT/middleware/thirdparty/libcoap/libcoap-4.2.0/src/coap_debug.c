...
coap_show_pdu(coap_log_t level, const coap_pdu_t *pdu) {
  ...
  unsigned char *buf = malloc(COAP_PDU_BUF_SIZE); /* need some space for output creation */
  ...
  char *outbuf = malloc(COAP_DEBUG_BUF_SIZE); 
  
  int outbuflen = 0;

  memset(buf, 0, COAP_PDU_BUF_SIZE);
  memset(outbuf, 0, COAP_DEBUG_BUF_SIZE);
  ...
  while ((option = coap_option_next(&opt_iter)) != NULL) {
    ...
    case COAP_SIGNALING_OPTION_MAX_MESSAGE_SIZE:
      buf_len = snprintf((char *)buf, COAP_PDU_BUF_SIZE, "%u",
    ...
    case COAP_OPTION_CONTENT_FORMAT:
      ...
      buf_len = print_content_format(content_format, buf, COAP_PDU_BUF_SIZE);
      break;
    ...
    case COAP_OPTION_BLOCK2:
      ...
      buf_len = snprintf((char *)buf, COAP_PDU_BUF_SIZE, "%u/%c/%u",
  ...
  if (coap_get_data(pdu, &data_len, &data)) {
    ...
    if (is_binary(content_format)) {
       ...
       */
      outbuf[0] = '\000';
      snprintf(outbuf, COAP_DEBUG_BUF_SIZE,  "<<");
      while (data_len--) {
        outbuflen = strlen(outbuf);
        snprintf(&outbuf[outbuflen], COAP_DEBUG_BUF_SIZE-outbuflen,
                 "%02x", *data++);
      ...
      if (print_readable(data, data_len, buf, sizeof(buf), 0)) {
        outbuflen = strlen(outbuf);
        snprintf(&outbuf[outbuflen], COAP_DEBUG_BUF_SIZE-outbuflen,  "'%s'", buf);
      }
    }
  }
...
//    char message[8 + 1024 * 2]; /* O/H + Max packet payload size * 2 */
    char message[128];          //wqzhao: dump too long message to unilog may case phy assert
#endif
    va_list ap;
    va_start(ap, format);
