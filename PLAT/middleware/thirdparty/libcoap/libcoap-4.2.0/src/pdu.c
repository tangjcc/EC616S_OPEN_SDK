...
coap_new_pdu(const struct coap_session_t *session) {
  ...
  unsigned int addr = __GET_RETURN_ADDRESS();
	ECOMM_TRACE(UNILOG_COAP, coap_msg_46, P_INFO, 3, "...coap_new_pdu.pdu=0x%x.....%d..lr=0x%x..", pdu, coap_send_packet_count, addr);
#ifndef NDEBUG
  if (!pdu)
...
coap_delete_pdu(coap_pdu_t *pdu) {
  ...
  unsigned int addr = __GET_RETURN_ADDRESS();
	ECOMM_TRACE(UNILOG_COAP, coap_msg_47, P_INFO, 3, ".coap_delete_pdu....pdu=0x%x....%d..lr=0x%x..", pdu,coap_send_packet_count, addr);
