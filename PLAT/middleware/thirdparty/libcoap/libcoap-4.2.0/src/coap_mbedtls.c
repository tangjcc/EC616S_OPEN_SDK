...
#ifdef HAVE_MBEDTLS //HAVE_MBEDTLS
  ...
  if (crt) {
    ...
    if (crt->ext_types & MBEDTLS_X509_EXT_SUBJECT_ALT_NAME) {
      ...
      if (seq) {
        ...
        char *buf = NULL;
        buf = malloc(seq->buf.len+1);
        memset(buf, 0, (seq->buf.len+1));
        memcpy(buf, seq->buf.p, seq->buf.len);

        return buf;
        //return (char *)mbedtls_strndup((const char *)seq->buf.p,
        //                     seq->buf.len);
      }
    }
    ...
    if (cn_data) {
      ...
      char *val = NULL;
      val = malloc(cn_data->val.len+1);
      memset(val, 0, (cn_data->val.len+1));
      memcpy(val, cn_data->val.p, cn_data->val.len);
      
      return val;   
      //return (char *)mbedtls_strndup((const char *)cn_data->val.p,
      //                       cn_data->val.len);
    }
  }
  return NULL;
...
const int set_psk_cipher[1] = {MBEDTLS_TLS_PSK_WITH_AES_128_CBC_SHA};
...
set_ciphersuites(mbedtls_ssl_config *conf, coap_enc_method_t method)
{
      ...
      if (cur) {
        if (cur->max_minor_ver < MBEDTLS_SSL_MINOR_VERSION_3) {
          /* Minimum of TLS1.2 required - skip */
            ...
            ECOMM_STRING(UNILOG_COAP, coap_ssl_str_0, P_INFO, "...cur..name..%s..", (uint8_t*)cur->name);
            ECOMM_TRACE(UNILOG_COAP, coap_ssl_0, P_INFO, 3, "...cur..cipher=%d..mac=%d.key_exchange=%d..",cur->cipher,cur->mac,cur->key_exchange);
      ...
      if (cur) {
        if (cur->max_minor_ver < MBEDTLS_SSL_MINOR_VERSION_3) {
          /* Minimum of TLS1.2 required - skip */
            ...
            ECOMM_STRING(UNILOG_COAP, coap_ssl_str_1, P_INFO, "..1.cur..name..%s..", (uint8_t*)cur->name);
            
            ECOMM_TRACE(UNILOG_COAP, coap_ssl_1, P_INFO, 3, "..1..cur..cipher=%d..mac=%d.key_exchange=%d..",cur->cipher,cur->mac,cur->key_exchange);
  ...
  mbedtls_ssl_conf_ciphersuites(conf, set_psk_cipher);
  
  //mbedtls_ssl_conf_ciphersuites(conf, method == COAP_ENC_PSK ? (const int *)psk_ciphers : pki_ciphers);
}
                                    ...
                                    coap_mbedtls_env_t *m_env)
{
  ...
  if (m_context->psk_pki_enabled & IS_PSK) {
    ...
    mbedtls_ssl_conf_psk(&m_env->conf, (const unsigned char *)psk_key,
                         ...
                         identity_len);
    if (c_session->mbedtls_cpsk_setup_data.client_sni) {
      mbedtls_ssl_set_hostname(&m_env->ssl,
                               c_session->mbedtls_cpsk_setup_data.client_sni);
    }
                                                     ...
                                                     coap_dtls_role_t role)
{
  ...
  ECOMM_TRACE(UNILOG_COAP, coap_ssl_2, P_VALUE, 2, "..coap_dtls_new_mbedtls_env...c_session=0x%x..role=%d.", c_session, role);           
  ...
  if ((ret = mbedtls_ctr_drbg_seed(&m_env->ctr_drbg,
                  mbedtls_entropy_func, &m_env->entropy, "IoTx", strlen("IoTx"))) != 0) {
...
#if 0
  mbedtls_ssl_set_bio(&m_env->ssl, c_session, mbedtls_net_send,
                                   ...
                                   const char *ca_path)
{
  ...
  if (ca_file) {
    //m_context->root_ca_file = (char *)mbedtls_strdup(ca_file);
    int len = strlen(ca_file);
    m_context->root_ca_file = (char *)mbedtls_malloc(len+1);
    memset(m_context->root_ca_file, 0, (len+1));
    memcpy(m_context->root_ca_file, ca_file, (len));
  }
  ...
  if (ca_path) {
    //m_context->root_ca_path = (char *)mbedtls_strdup(ca_path);

    int path_len = strlen(ca_path);
    m_context->root_ca_path = (char *)mbedtls_malloc(path_len+1);
    memset(m_context->root_ca_path, 0, (path_len+1));
    memcpy(m_context->root_ca_path, ca_path, (path_len));  
  }
  return 1;
}
                   ...
                   size_t data_len)
{
  ...
  m_env->ssl.rai = c_session->raiFlag;
...
{
    ECOMM_TRACE(UNILOG_COAP, coap_ssl_3, P_INFO, 2, "...coap_dtls_send=%d..0x%x..",i,data[i]);
}
                      ...
                      size_t data_len)
{
  ...
  ECOMM_TRACE(UNILOG_COAP, coap_ssl_4, P_INFO, 0, "..coap_dtls_receive.....\r\n");
  ...
  if (m_env->established) {
    ...
    ECOMM_TRACE(UNILOG_COAP, coap_ssl_5, P_INFO, 2, "..coap_dtls_receive..ret=0x%x..=%d.\r\n",ret,ret);
...
coap_tls_version_t * coap_get_tls_library_version(void)
{
  ...
  version.version = MBEDTLS_VERSION_NUMBER;//mbedtls_version_get_number();
