...
#if !defined(WITH_TINYDTLS_COAP) && !defined(HAVE_OPENSSL) && !defined(HAVE_LIBGNUTLS) && !defined(HAVE_MBEDTLS)
...
coap_tick_t coap_dtls_get_timeout(coap_session_t *session UNUSED, coap_tick_t now) {
  return 0;
}
...
#else /* !WITH_TINYDTLS_COAP && !HAVE_OPENSSL && !HAVE_LIBGNUTLS */
...
#endif /* !WITH_TINYDTLS_COAP && !HAVE_OPENSSL && !HAVE_LIBGNUTLS */
