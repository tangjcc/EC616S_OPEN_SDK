...
ssize_t coap_session_send(coap_session_t *session, const uint8_t *data, size_t datalen)
{
  ...
  ECOMM_TRACE(UNILOG_COAP, coap_msg_30, P_INFO, 3, ".....coap_session_send..%d..delayqueue=0x%x..lr=0x%x..", session->state,session->delayqueue, addr);
                       ...
                       coap_queue_t *node)
{
	...
	unsigned int addr = __GET_RETURN_ADDRESS();
	ECOMM_TRACE(UNILOG_COAP, coap_msg_31, P_INFO, 6, ".coap_session_delay_pdu..pdu=0x%x.sendqueue=0x%x...delayqueue=0x%x...%d...node->id=%d.lr=0x%x..", 
		pdu, session->context->sendqueue,session->delayqueue,session->state, node->id,addr);
  ...
  ECOMM_TRACE(UNILOG_COAP, coap_msg_32, P_INFO, 4, ".....coap_session_delay_pdu.%d.sendqueue=0x%x..delayqueue=0x%x.lr=0x%x..", 
  	session->state, session->context->sendqueue,session->delayqueue,addr);
...
void coap_session_connected(coap_session_t *session) {
	...
	ECOMM_TRACE(UNILOG_COAP, coap_msg_33, P_INFO, 4, ".....coap_session_connected..%d.sendqueue=0x%x..delayqueue=0x%x..lr=0x%x..", 
		session->state,session->context->sendqueue,session->delayqueue, addr);
  if (session->state != COAP_SESSION_STATE_ESTABLISHED) {
...
coap_session_t *coap_new_client_session_psk(
  ...
  setup_data = malloc(sizeof(coap_dtls_cpsk_t));
