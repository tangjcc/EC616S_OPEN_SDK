...
dtls_send_to_peer(struct dtls_context_t *dtls_context,
  session_t *dtls_session, uint8 *data, size_t len, uint8_t raiflag) {
...
static dtls_handler_t cb = {
  .dtlswrite = dtls_send_to_peer,
  .dtlsread = dtls_application_data,
  .event = dtls_event,
...
coap_dtls_send(coap_session_t *session,
  ...
  res = dtls_write((struct dtls_context_t *)session->context->dtls_context,
    (session_t *)session->tls, data_rw, data_len, PS_SOCK_RAI_NO_INFO);
