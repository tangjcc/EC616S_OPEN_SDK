...
ssize_t coap_network_send(coap_socket_t *sock, const coap_session_t *session, const uint8_t *data, size_t datalen)
{
  ...
  if (!coap_debug_send_packet()) {
...
#else
    #if LIBCOAP_RAI_OPTIMIZE == 1
    bytes_written = ps_send(sock->fd, data, datalen, 0, session->raiFlag, false);
    #else
...
#ifndef WITH_POSIX  //WITH_CONTIKI
    ...
    //sendto(sock->fd, data, datalen, 0, (struct sockaddr*)&session->remote_addr.addr.sa., sizeof(struct sockaddr));
...
coap_network_read(coap_socket_t *sock, coap_packet_t *packet) {
  ...
  if (sock->flags & COAP_SOCKET_CONNECTED) {
    ...
    int r;
#endif
#ifndef WITH_POSIX  //#if !defined(WITH_CONTIKI)
    /* a buffer large enough to hold all packet info types, ipv6 is the largest */
    char buf[CMSG_SPACE(sizeof(struct in6_pktinfo))];
    struct msghdr mhdr;
    struct iovec iov[1];
           ...
           coap_tick_t now)
{
  ...
  *num_sockets = 0;
  ...
  ECOMM_TRACE(UNILOG_COAP, coap_write_0, P_INFO, 6, "...coap_write..now=%d..basetime=%d..nextpdu->t=%d..rangtime=%d..sendqueue=nextpdu=0x%x..delayqueue=0x%x.",now,ctx->sendqueue_basetime,nextpdu->t,(now - ctx->sendqueue_basetime),nextpdu,ctx->sessions->delayqueue);
...
int
coap_run_once(coap_context_t *ctx, unsigned timeout_ms, void *pClient) {
  ...
  #if COAP_SLP_ENABLE
  coapAddTimeoutInfo(pClient, timeout);
  #endif
  ...
  result = select(nfds, &readfds, &writefds, &exceptfds, (void *)(timeout > 0 ? &tv : NULL));
...
int coap_autoreg_run_once(coap_context_t *ctx, unsigned timeout_ms, void *pClient) {
  fd_set readfds, writefds, exceptfds;
  coap_fd_t nfds = 0;
  struct timeval tv;
  coap_tick_t before, now;
  ...
  const uint8_t *data, size_t data_len)
{
	...
	coap_send_packet_count_send++;
	ECOMM_TRACE(UNILOG_COAP, coap_socket_send0, P_INFO, 5, ".....coap_socket_send...pdu_id=%d.%d.delayqueue=0x%x..lr=0x%x....%d.", session->context->sendqueue->pdu->tid,session->state,session->delayqueue, addr,coap_send_packet_count_send);
