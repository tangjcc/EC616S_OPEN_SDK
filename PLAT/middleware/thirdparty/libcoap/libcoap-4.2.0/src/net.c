...
//extern Mutex coap_mutex2;
//extern int coap_mutex_lock(Mutex* mutex);
//extern int coap_mutex_unlock(Mutex* mutex);
...
unsigned int addr = __GET_RETURN_ADDRESS();
  ECOMM_TRACE(UNILOG_COAP, coap_msg_34, P_INFO, 3, "..coap_delete_node...node=0x%x..node->id=%d...lr=0x%x..", node, node->id, addr);
...
coap_delete_all(coap_queue_t *queue) {
  if (!queue)
    return;
  unsigned int addr = __GET_RETURN_ADDRESS();
	ECOMM_TRACE(UNILOG_COAP, coap_msg_35, P_INFO, 3, ".coap_delete_all..queue=0x%x...queue->id=%d....lr=0x%x..", queue, queue->id, addr);
...
coap_new_node(void) {
  ...
  unsigned int addr = __GET_RETURN_ADDRESS();
	ECOMM_TRACE(UNILOG_COAP, coap_msg_36, P_INFO, 2, ".coap_new_node..node=0x%x.....lr=0x%x..", node, addr);
...
coap_session_send_pdu(coap_session_t *session, coap_pdu_t *pdu) {
  ...
  ECOMM_TRACE(UNILOG_COAP, coap_msg_37, P_INFO, 6, ".....coap_session_send_pdu..%d..sendqueue=0x%x...delayqueue=0x%x..pdu..=0x%x.pdu->id=%d..lr=0x%x..", 
  	session->state,session->context->sendqueue,session->delayqueue, pdu, pdu->tid,addr);
...
coap_send(coap_session_t *session, coap_pdu_t *pdu) {
  ...
  coap_mutex_lock(&coap_mutex2);

  if (!coap_pdu_encode_header(pdu, session->proto)) {
    coap_mutex_unlock(&coap_mutex2);
    goto error;
  }
  ...
  coap_tid = coap_wait_ack(session->context, session, node);
  coap_mutex_unlock(&coap_mutex2);
  return coap_tid;
 error:
  coap_delete_pdu(pdu);
...
coap_retransmit(coap_context_t *context, coap_queue_t *node) {
	...
	unsigned int addr = __GET_RETURN_ADDRESS();
	  ECOMM_TRACE(UNILOG_COAP, coap_msg_38, P_INFO, 1, ".....coap_retransmit.....lr=0x%x..", addr);

  if (!context || !node)
    return COAP_INVALID_TID;
  ECOMM_TRACE(UNILOG_COAP, coap_msg_39, P_INFO, 5, "..coap_retransmit.node=0x%x..retransmit_cnt...%d.con_active=%d..sendqueue=0x%x..delayqueue=0x%x..", node, node->retransmit_cnt,node->session->con_active,context->sendqueue,node->session->delayqueue);
...
coap_write_session(coap_context_t *ctx, coap_session_t *session, coap_tick_t now) {
  ...
  unsigned int addr = __GET_RETURN_ADDRESS();
	ECOMM_TRACE(UNILOG_COAP, coap_msg_40, P_INFO, 3, ".....coap_write_session...sendqueue=0x%x..delayqueue=0x%x....lr=0x%x..", ctx->sendqueue,session->delayqueue, addr);
...
void
coap_cancel_curr_messages(coap_context_t *context, coap_session_t *session,coap_pdu_t *sent,coap_pdu_t *rcvd,
  ...
  coap_queue_t *p, *q;
  ECOMM_TRACE(UNILOG_COAP, coap_msg_41, P_INFO, 6, "...coap_cancel_curr_messages.sendqueue=0x%x..delayqueue=0x%x..token=0x%x..token_length=%d...sent->tid=%d..rcvd->tid=%d.", 
  	context->sendqueue,session->delayqueue, token, token_length,sent->tid,rcvd->tid);

  while (context->sendqueue && context->sendqueue->session == session &&
    (context->sendqueue->id == rcvd->tid)) {
    q = context->sendqueue;
    context->sendqueue = q->next;
    coap_log(LOG_DEBUG, "** %s: tid=%d: removed\n",
             coap_session_str(session), q->id);
...
coap_cancel_all_messages(coap_context_t *context, coap_session_t *session,
   ...
   * and use the specified token */
  coap_queue_t *p, *q;
  ECOMM_TRACE(UNILOG_COAP, coap_msg_42, P_INFO, 4, "...coap_cancel_all_messages.sendqueue=0x%x..delayqueue=0x%x..token=0x%x..token_length=%d..", 
  	context->sendqueue,session->delayqueue, token, token_length);
...
handle_response(coap_context_t *context, coap_session_t *session,
  ...
  ECOMM_TRACE(UNILOG_COAP, coap_msg_43, P_INFO, 6, "...handle_response.sendqueue=0x%x..delayqueue=0x%x..sent=0x%x..rcvd=0x%x..context=0x%x...session=0x%x", 
  	context->sendqueue,session->delayqueue, sent, rcvd,context,session);
   ...
   * same token.
   */
  coap_cancel_curr_messages(context, session, sent, rcvd, rcvd->token, rcvd->token_length);
...
#define INET6_ADDRSTRLEN 40
  ...
  unsigned int addr = __GET_RETURN_ADDRESS();
  coap_send_packet_count_recv++;
  ECOMM_TRACE(UNILOG_COAP, coap_msg_44, P_INFO, 8, ".....coap_dispatch..pdu->tid=%d.pdu->type=%d..pdu->code=%d.%d...sendqueue=0x%x..delayqueue=0x%x..lr=0x%x....%d.", 
  	pdu->tid,pdu->type,pdu->code,session->state,session->context->sendqueue,session->delayqueue, addr,coap_send_packet_count_recv);
  ECOMM_TRACE(UNILOG_COAP, coap_msg_45, P_INFO, 4, ".....coap_dispatch..session->con_active=%d..context->sendqueue=0x%x.session_sendqueue=0x%x..delayqueue=0x%x.", 
  	session->con_active,context->sendqueue,session->context->sendqueue,session->delayqueue);
