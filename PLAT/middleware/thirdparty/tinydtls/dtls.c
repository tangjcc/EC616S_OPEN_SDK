...
trace_add_module(TINYDTLS, P_DEBUG);
...
dtls_send_multi(dtls_context_t *ctx, dtls_peer_t *peer,
        ...
        size_t buf_len_array[], size_t buf_array_len, uint8_t raiflag);
...
dtls_send(dtls_context_t *ctx, dtls_peer_t *peer, unsigned char type,
      uint8 *buf, size_t buflen, uint8_t raiflag) {
  ECOMM_TRACE(UNILOG_ONENET, dtls_send, P_INFO, 0, "call dtls_send_multi");
...
dtls_write(struct dtls_context_t *ctx, 
       session_t *dst, uint8 *buf, size_t len, uint8_t raiflag) {
  
  ECOMM_TRACE(UNILOG_ONENET, dtls_write_1, P_INFO, 0, "dtls_write");
  dtls_peer_t *peer = dtls_get_peer(ctx, dst);

  /* Check if peer connection already exists */
  if (!peer) { /* no ==> create one */
    ECOMM_TRACE(UNILOG_ONENET, dtls_write_2, P_INFO, 0, "dtls::peer == NULL");
    int res;
    ...
    if (peer->state != DTLS_STATE_CONNECTED) {
      ECOMM_TRACE(UNILOG_ONENET, dtls_write_2_1, P_INFO, 1, "peer->state=%d,not connected return 0",peer->state);
      return 0;
    } else {
      ECOMM_TRACE(UNILOG_ONENET, dtls_write_3, P_INFO, 0, "call dtls_send");
...
{
  ECOMM_TRACE(UNILOG_ONENET, dtls_check_tls_extension_1, P_INFO, 1, "enter client_hello=%d,peer->state=%d",client_hello,peer->state);
  uint16_t i, j;
  ...
  if (data_length < sizeof(uint16)) { 
    ...
    if (is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(handshake->cipher)) {
      ECOMM_TRACE(UNILOG_ONENET, dtls_check_tls_extension_2, P_INFO, 0, "is_tls_ecdhe_ecdsa_with_aes_128_ccm_8");
      goto error;
    }
    return 0;
  }
  ...
  while (data_length) {
    ...
    switch (i) {
      ...
      if (dtls_uint8_to_int(data) != 0)//EC add TLS_CERT_TYPE_RAW_PUBLIC_KEY to 0 for onenet
#else
        ...
        size_t buf_len_array[], size_t buf_array_len, uint8_t raiflag)
{
  ...
  ECOMM_TRACE(UNILOG_ONENET, dtls_send_multi, P_INFO, 1, "call dtlswrite also dtlsconnection's send_to_peer len=%d",len);
  res = CALL(ctx, dtlswrite, session, sendbuf, len, raiflag);
...
check_server_hello(dtls_context_t *ctx, 
              ...
              uint8 *data, size_t data_length)
{
  ECOMM_TRACE(UNILOG_ONENET, check_server_hello_1, P_INFO, 1, "enter");
  dtls_handshake_parameters_t *handshake = peer->handshake_params;
                           ...
                           uint8      **cleartext )
{
    ...
    *cleartext = (uint8 *)packet + sizeof(dtls_record_header_t);
    ...
    if ( !security )
    {
        ECOMM_TRACE(UNILOG_ONENET, decrypt_verify_1, P_INFO, 1, "No security context for epoch:%d return -1", dtls_get_epoch( header ));
    ...
    if ( security->cipher == TLS_NULL_WITH_NULL_NULL ){
        ...
        ECOMM_TRACE(UNILOG_ONENET, decrypt_verify_2, P_INFO, 1, "no cipher suite selected return clear data len %d",clen);
        return clen;
    } else {
        /** 
        * length of additional_data for the AEAD cipher which consists of
        ...
        */
        ...
        if ( clen < 16 ){       /* need at least IV and MAC */
            ECOMM_TRACE(UNILOG_ONENET, decrypt_verify_3, P_INFO, 0, "cleare len < 16");
            return -1;
        }
        memset(nonce, 0, DTLS_CCM_BLOCKSIZE);
        ...
        */
        ...
        if ( clen < 0 ){
            ...
            ECOMM_TRACE(UNILOG_ONENET, decrypt_verify_4, P_INFO, 0, "decryption failed");
        }
        else
        {
...
dtls_renegotiate(dtls_context_t *ctx, const session_t *dst)
{
    ...
    if ( peer->role == DTLS_CLIENT )
    {
        ECOMM_TRACE(UNILOG_ONENET, dtls_renegotiate_1, P_INFO, 0, "role is client.send ClientHello");
        /* send ClientHello with empty Cookie */
        err = dtls_send_client_hello(ctx, peer, NULL, 0);
        if ( err < 0 ){
            dtls_warn( "cannot send ClientHello\n" );
            ECOMM_TRACE(UNILOG_ONENET, dtls_renegotiate_2, P_INFO, 0, "cannot send ClientHello");
        }
        else
    ...
    {
        ECOMM_TRACE(UNILOG_ONENET, dtls_renegotiate_3, P_INFO, 0, "role is server.send hello request");
...
handle_handshake_msg(dtls_context_t *ctx, dtls_peer_t *peer, session_t *session,
  ...
  int err = 0;
  ECOMM_TRACE(UNILOG_ONENET, handle_handshake_msg_1, P_INFO, 3, "data[0]=%d, state=%d, data_length=%d",data[0],state,data_length);
  ...
  case DTLS_HT_SERVER_HELLO:
    ...
    if (err < 0) {
      ECOMM_TRACE(UNILOG_ONENET, handle_handshake_msg_2, P_INFO, 1, "error in check_server_hello err: %d",err);
         ...
         uint8 *data, size_t data_length)
{
  ...
  ECOMM_TRACE(UNILOG_ONENET, handle_handshake_1, P_INFO, 1, "data length=%d",data_length);
  ...
  ECOMM_STRING(UNILOG_ONENET, handle_handshake_2, P_INFO, "received handshake packet of type: %s",(uint8_t*)dtls_handshake_type_to_name( hs_header->msg_type ));
  ...
  if (!peer || !peer->handshake_params) {
    ...
    if (hs_header->msg_type != DTLS_HT_CLIENT_HELLO && !peer) {
      ...
      ECOMM_TRACE(UNILOG_ONENET, handle_handshake_3, P_INFO, 2, "hs_header->msg_type=%d peer=%x", hs_header->msg_type, peer);
      return dtls_alert_fatal_create(DTLS_ALERT_HANDSHAKE_FAILURE);
    }
...
dtls_handle_message(dtls_context_t *ctx, 
  ...
  if (!peer) {
    ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_1, P_INFO, 0, "PEER NOT FOUND");
    dtls_debug("dtls_handle_message: PEER NOT FOUND\n");
    dtls_dsrv_log_addr(DTLS_LOG_DEBUG, "peer addr", session);
  } else {
    ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_2, P_INFO, 0, "FOUND PEER");
    dtls_debug("dtls_handle_message: FOUND PEER\n");
    if(peer->security_params[0])
      ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_2_1, P_INFO, 1, "peer.s0.epoch=%d",peer->security_params[0]->epoch);
    if(peer->security_params[1])
      ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_2_2, P_INFO, 1, "peer.s1.epoch=%d",peer->security_params[1]->epoch);
  }
  ...
  while (rlen) {
    ...
    ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_3, P_INFO, 2, "got packet %d (%d bytes)", msg[0], rlen);
    if (peer) {
      ...
      if (!security) {
        dtls_alert("No security context for epoch: %i\n", dtls_get_epoch(header));
        ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_4, P_INFO, 1, "No security context for epoch: %d", dtls_get_epoch(header));
        data_length = -1;
      } else {
        ...
        if(pkt_seq_nr == 0 && security->cseq.cseq == 0) {
          ...
          ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_5, P_INFO, 0, "Duplicate packet arrived return 0");
          return 0;
          ...
          if (((security->cseq.cseq-1)-pkt_seq_nr) < 64) {
              if(security->cseq.bitfield & (1<<((security->cseq.cseq-1)-pkt_seq_nr))) {
                ...
                ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_6, P_INFO, 0, "Duplicate packet arrived (bitfield) return 0");
                /* seen it */
                  return 0;
              } else {
                ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_7, P_INFO, 0, "Packet arrived out of order");
            ...
            ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_8, P_INFO, 0, "Packet from before the bitfield arrived");
          ...
          if(data_length > 0) {
            ...
            ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_9, P_INFO, 2, "new packet arrived with seq_nr: %x, new bitfield is:%x",pkt_seq_nr,security->cseq.bitfield);
      ...
      if (data_length < 0) {
          if (hs_attempt_with_existing_peer(msg, rlen, peer)) {
              ...
              ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_10, P_INFO, 1, "peer->state=%d",peer->state);
              ...
              //EC add begin
              if (peer->state == DTLS_STATE_CONNECTED) {
                  ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_10_1, P_INFO, 0, "peer->state == DTLS_STATE_CONNECTED session delete by server, destroy peer for recreate it");
                  peer->state = DTLS_STATE_CLOSED;
                  dtls_destroy_peer(ctx, peer, 1);
              }
              //EC add end
              ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_11, P_INFO, 1, "DTLS_ALERT_DECRYPT_ERROR return err=%d",err);
              return err;
          }
      } else {
          role = peer->role;
    ...
    case DTLS_CT_CHANGE_CIPHER_SPEC:
      ...
      if (err < 0) {
        ...
        ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_12, P_INFO, 1, "error while handling ChangeCipherSpec message return err=%d", err );
        return err;
      }
      break;
    ...
    case DTLS_CT_ALERT:
      ...
      if (err < 0 || err == 1) {
         ...
         peer = NULL;
         ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_13, P_INFO, 1, "received alert, peer has been invalidated return err=%d", err );
         return err < 0 ?err:-1;
      }
      break;
    ...
    case DTLS_CT_HANDSHAKE:
          ...
          */
          ...
          if (expected_epoch != msg_epoch) {
              if (hs_attempt_with_existing_peer(msg, rlen, peer)) {
                  ...
                  ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_14, P_INFO, 2, "Wrong epoch, expected %d, got: %d", expected_epoch, msg_epoch);
                  break;
              }
          }
      }
      ...
      if (err < 0) {
          ...
          dtls_alert_send_from_err(ctx, peer, session, err);
          ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_15, P_INFO, 1, "error while handling handshake packet return err=%d", err );
          return err;
      }
      if (peer && peer->state == DTLS_STATE_CONNECTED) {
          /* stop retransmissions */
          ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_16, P_INFO, 0, "DTLS_EVENT_CONNECTED stop retransmission");
          dtls_stop_retransmission(ctx, peer);
    ...
    case DTLS_CT_APPLICATION_DATA:
      ...
      if (!peer) {
        ...
        ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_17, P_INFO, 0, "no peer available, return -1" );
        // TODO: should we send a alert here?
        return -1;
      }
      ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_18, P_INFO, 0, "DTLS_CT_APPLICATION_DATA stop retransmission and read");
      dtls_stop_retransmission(ctx, peer);
      CALL(ctx, dtlsread, &peer->session, data, data_length);
      break;
    default:
      ECOMM_TRACE(UNILOG_ONENET, dtls_handle_message_19, P_INFO, 1, "dropped unknown message of type %d",msg[0]);
...
dtls_connect_peer(dtls_context_t *ctx, dtls_peer_t *peer) {
  ...
  if (peer == dtls_get_peer(ctx, &peer->session)) {
    ECOMM_TRACE(UNILOG_ONENET, dtls_connect_peer_2, P_INFO, 0, "found peer,already in our list, try to re-negotiate");
  ...
  peer->handshake_params->hs_state.mseq_r = 0;
  peer->handshake_params->hs_state.mseq_s = 0;
  ECOMM_TRACE(UNILOG_ONENET, dtls_connect_peer_1, P_INFO, 0, "add new peer in our list, send client hello");
  res = dtls_send_client_hello(ctx, peer, NULL, 0);
  if ( res < 0 ){
    ECOMM_TRACE(UNILOG_ONENET, dtls_connect_peer_3, P_INFO, 0, "cannot send ClientHello");
    dtls_warn( "cannot send ClientHello\n" );
  }
  else 
    peer->state = DTLS_STATE_CLIENTHELLO;
...
dtls_connect(dtls_context_t *ctx, const session_t *dst) {
  ...
  if (!peer){
    ECOMM_TRACE(UNILOG_ONENET, dtls_connect_1, P_INFO, 0, "peer is NULL new a peer");
    peer = dtls_new_peer(dst);
  }
  if (!peer) {
    ECOMM_TRACE(UNILOG_ONENET, dtls_connect_2, P_INFO, 0, "cannot create new peer");
    dtls_crit("cannot create new peer\n");
    return -1;
  }

  ECOMM_TRACE(UNILOG_ONENET, dtls_connect_3, P_INFO, 0, "to connect this peer");
   ...
   * re-negotiation. */
  if (res > 0) {
    ECOMM_TRACE(UNILOG_ONENET, dtls_connect_3_1, P_INFO, 0, "CALL cis_dtls_event DTLS_EVENT_CONNECT");
    CALL(ctx, event, &peer->session, (dtls_alert_level_t)0, DTLS_EVENT_CONNECT);
  } else if (res == 0) {
    ECOMM_TRACE(UNILOG_ONENET, dtls_connect_3_2, P_INFO, 0, "CALL cis_dtls_event DTLS_EVENT_RENEGOTIATE");
...
static void
dtls_retransmit(dtls_context_t *context, netq_t *node, uint8_t raiflag) {
  if (!context || !node)
    return;

  /* re-initialize timeout when maximum number of retransmissions are not reached yet */
  if (node->retransmit_cnt < DTLS_DEFAULT_MAX_RETRANSMIT) {
      ECOMM_TRACE(UNILOG_ONENET, dtls_retransmit_1, P_INFO, 1, "%d,no reach the maximum retry time",node->retransmit_cnt);
      unsigned char sendbuf[DTLS_MAX_BUF];
      size_t len = sizeof(sendbuf);
      int err;
      ...
      if (node->type == DTLS_CT_HANDSHAKE) {
        ...
        ECOMM_STRING(UNILOG_ONENET, dtls_retransmit_2, P_INFO,  "** retransmit handshake packet of type: %s ",(uint8_t*)dtls_handshake_type_to_name( DTLS_HANDSHAKE_HEADER(data)->msg_type));
      ...
      (void)CALL(context, dtlswrite, &node->peer->session, sendbuf, len, raiflag);
      return;
  }
