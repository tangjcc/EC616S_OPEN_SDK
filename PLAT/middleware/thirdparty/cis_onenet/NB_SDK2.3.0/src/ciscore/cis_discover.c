                                  ...
                                  size_t bufferLen )
{
    ...
    switch ( tlvP->type )
    {
        ...
        //case cis_data_type_object_instance:
        {
            ...
            res = cis_utils_intToText( tlvP->id, buffer + head, bufferLen - head );
            if ( res <= 0 ) return -1;
            head += res;
            ...
            if ( tlvP->type == DATA_TYPE_OBJECT_INSTANCE )
            {
                ...
                res = cis_utils_intToText( tlvP->value.asChildren.count, buffer + head, bufferLen - head );
                if ( res <= 0 ) return -1;
                head += res;
		...
		case DATA_TYPE_OBJECT:
        {
            ...
            res = cis_utils_intToText( tlvP->id, uriStr + uriLen, URI_MAX_STRING_LEN - uriLen );
            if ( res <= 0 ) return -1;
            uriLen += res;
                        ...
                        uint8_t ** bufferP )
{
    ...
    res = cis_utils_intToText( uriP->objectId, bufferLink + head, PRV_LINK_BUFFER_SIZE - head );
    ...
    if ( CIS_URI_IS_SET_INSTANCE( uriP ) )
    {
        ...
        res = cis_utils_intToText( uriP->objectId, bufferLink + head + subHead, PRV_LINK_BUFFER_SIZE - head - subHead );
        if ( res <= 0 ) goto TAG_FAILED;
        subHead += res;
        PRV_CONCAT_STR( bufferLink + head, PRV_LINK_BUFFER_SIZE - head, subHead, LINK_URI_SEPARATOR, LINK_URI_SEPARATOR_SIZE );
        res = cis_utils_intToText( uriP->instanceId, bufferLink + head + subHead, PRV_LINK_BUFFER_SIZE - head - subHead );
        if ( res <= 0 ) goto TAG_FAILED;
        subHead += res;
