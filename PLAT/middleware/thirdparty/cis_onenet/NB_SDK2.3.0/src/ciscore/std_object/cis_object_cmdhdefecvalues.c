                         ...
                         st_object_t * cmdhdefecvaluesObj)
{
    ...
    ECOMM_TRACE(UNILOG_ONENET, std_cmdhdefecvalues_create_0, P_INFO, 0, "cmdhdefecvalues object create finished.");
        ...
        case RES_M_REQUESTORIGIN:
			{
				...
				if((data->asBuffer.length > PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTAREQ_LEN)&&(data->asBuffer.length < PRV_CMDHDEFECVALUES_REQUESTORIGIN_MAX_LEN))
				{
                    ...
                    ECOMM_TRACE(UNILOG_ONENET, std_cmdhdefecvalues_write_1, P_INFO, 2, "write cmdhdefecvalues object: %d, resid: %d,bufferlen: %d",
                                objectP->objID,data->id,data->asBuffer.length);
					LOGD("Received Command Detail : %s \r\n", data->asBuffer.buffer);
                    ECOMM_STRING(UNILOG_ONENET, std_cmdhdefecvalues_write_2, P_VALUE, "Received Command Detail : %s", data->asBuffer.buffer);
					...
					{	
                        ...
                        ECOMM_STRING(UNILOG_ONENET, std_cmdhdefecvalues_write_3, P_VALUE, "CMIOT:OTAREQ received. Series Num : %s", data->asBuffer.buffer+PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTAREQ_LEN);
                        cis_memset(targetP->RequestContext, 0, PRV_CMDHDEFECVALUES_REQUESTCONTEXT_MAX_LEN);
                        ...
                        ECOMM_STRING(UNILOG_ONENET, std_cmdhdefecvalues_write_31, P_WARNING, "onenet SIM OTA:OTAREQ received. notify : %s", (uint8_t *)targetP->RequestContext);
						...
						strncmp((const char *)targetP->RequestOrigin,PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTAFINISH_STR_CAPITALS,PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTAFINISH_LEN) == 0)
					{
                        ...
                        ECOMM_STRING(UNILOG_ONENET, std_cmdhdefecvalues_write_4, P_WARNING, "CMIOT:OTAFINISH received. Result Num : %s", data->asBuffer.buffer+PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTAFINISH_LEN);
                        result = COAP_204_CHANGED;
						...
						strncmp((const char *)targetP->RequestOrigin,PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTASTART_STR_CAPITALS,PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTASTART_LEN) == 0)
					{
                        ...
                        ECOMM_STRING(UNILOG_ONENET, std_cmdhdefecvalues_write_5, P_WARNING, "onenet SIM OTA:OTASTART received. Series Num : %s", data->asBuffer.buffer+PRV_CMDHDEFECVALUES_REQUESTORIGIN_OTASTART_LEN);
                        result = COAP_204_CHANGED;
