                       ...
                       st_object_t * powerupLogObj)
{
	...
	uint8_t Imsi =	cissys_getIMSI(targetP->IMSI,NBSYS_IMSI_MAXLENGTH, true);
    ...
    ECOMM_TRACE(UNILOG_ONENET, std_poweruplog_create_0, P_INFO, 0, "Power up log create finished.");
			...
			case RES_M_IMSI:
				{
					...
					tmpdata->asBuffer.length = cissys_getIMSI((char *)tmpdata->asBuffer.buffer,NBSYS_IMSI_MAXLENGTH, true);
					cis_notify(contextP,&uri,tmpdata,(cis_mid_t)mid,CIS_NOTIFY_CONTINUE,false);
					cis_free(tmpdata->asBuffer.buffer);
					break;
				}
