		...
		case RES_M_PACKAGE:
		{
			...
			if (contextP->isupdate_mcu == false) {
				if (data->asBuffer.length == 0)//restart a new update task
				{
					...
					core_callbackEvent(contextP, CIS_EVENT_FIRMWARE_UPDATE_OVER, NULL);
			...
			if (data->asBuffer.length == 0)//restart a new update task
			{
				...
				ECOMM_TRACE(UNILOG_ONENET, firmware_write_3, P_INFO, 0, "RES_M_PACKAGE, ERASE ");                      
				#endif
                ...
                #ifdef CIS_DEBUG_MOD_ONENET                                                         
                ECOMM_TRACE(UNILOG_ONENET, firmware_write_1, P_INFO, 0, "erased asBuffer.length==0");  
                #endif
			}
			else
			{
                #ifdef CIS_DEBUG_MOD_ONENET               
                ECOMM_TRACE(UNILOG_ONENET, firmware_write_2, P_INFO, 0, "write  state %d", targetP->fw_info->state );
                #endif
	...
	st_object_t * objectP)
{
	...
	case RES_M_UPDATE:
		if (targetP->fw_info->state == FOTA_STATE_DOWNLOADED)
		{
			...
			{
				#ifdef CIS_DEBUG_MOD_ONENET                             
				ECOMM_TRACE(UNILOG_ONENET, excute_1, P_INFO, 1, "RES_M_UPDATE fw_info->result %d", targetP->fw_info->result);
				#endif
		...
		{
			#ifdef CIS_DEBUG_MOD_ONENET                             
			ECOMM_TRACE(UNILOG_ONENET, excute_3, P_INFO, 1, "RES_M_UPDATE fw_info->result %d", targetP->fw_info->result);
			#endif
				...
				|| targetP->fw_info->result == FOTA_RESULT_UPDATEFAIL)//bad request
			{
				...
				if (contextP->isupdate_mcu == false)
				{
					...
					if (!cissys_updateFirmware(&(contextP->g_fotacallback)))
						return COAP_500_INTERNAL_SERVER_ERROR;
					#ifdef CIS_DEBUG_MOD_ONENET                  
					ECOMM_TRACE(UNILOG_ONENET, excute_3_1, P_INFO, 0, "cis_reboot_timer");                                    
					#else
					ECOMM_TRACE(UNILOG_ATCMD_LWM2M, excute_3_1, P_INFO, 0, "cis_reboot_timer");
					#endif                                    
