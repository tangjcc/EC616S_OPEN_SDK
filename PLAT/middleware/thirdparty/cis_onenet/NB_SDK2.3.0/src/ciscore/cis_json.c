                              ...
                              size_t bufferLen)
{
    ...
    case DATA_TYPE_INTEGER:
    {
        ...
        res = cis_utils_intToText(value, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
    ...
    case DATA_TYPE_FLOAT:
    {
        ...
        res = cis_utils_floatToText(value, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
                      ...
                      uint8_t ** bufferP)
{
         ...
         || targetP->type == cis_data_type_multiple_resource))
    {
        ...
        res = cis_utils_intToText(targetP->id, baseUriStr + baseUriLen, URI_MAX_STRING_LEN - baseUriLen);
        if (res <= 0) return 0;
        baseUriLen += res;
