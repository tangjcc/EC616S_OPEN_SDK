                    ...
                    cis_time_t currentTime)
{
    ...
    if (targetP != NULL)
    {
        ECOMM_STRING(UNILOG_ONENET, bootstrap_step_1, P_INFO, "bootstrap step status: %s", (uint8_t*)STR_STATUS(targetP->status));
        ...
        ECOMM_STRING(UNILOG_ONENET, bootstrap_step_2, P_INFO, "bootstrap step status finish: %s", (uint8_t *)STR_STATUS(targetP->status));
                                     ...
                                     void * fromSessionH)
{
     ...
     && bootstrapServer->status == STATE_BS_PENDING)
    {
        ...
        transaction_removeAll(context);//EC add
...
void bootstrap_init(st_context_t * context)
{
    ...
    ECOMM_TRACE(UNILOG_ONENET, bootstrap_init_1, P_INFO, 0, "make serverlist");
    if (bootstrapServer != NULL)
    {
        bootstrapServer->status = STATE_UNCREATED;
        context->bootstrapServer = bootstrapServer;
        ECOMM_STRING(UNILOG_ONENET, bootstrap_init_2, P_INFO, "bootstrapServer=%s",(uint8_t *)bootstrapServer->host);
    }else{
        ECOMM_TRACE(UNILOG_ONENET, bootstrap_init_3, P_INFO, 0, "bootstrapServer=NULL");
...
void bootstrap_create(st_context_t * context)
{
    ...
    ECOMM_TRACE(UNILOG_ONENET, bootstrap_create_1, P_INFO, 0, "get bserver to create network");

    management_createNetwork(context,bootstrapServer);
    if(bootstrapServer->sessionH == NULL){
        ECOMM_TRACE(UNILOG_ONENET, bootstrap_create_2, P_INFO, 0, "create session failed !!!");
...
void  bootstrap_connect(st_context_t * context)
{
    ...
    ECOMM_TRACE(UNILOG_ONENET, bootstrap_connect_1, P_INFO, 0, "get bserver to connect network");
    if(!management_connectServer(context,bootstrapServer))
    {
        ECOMM_TRACE(UNILOG_ONENET, bootstrap_connect_2, P_INFO, 0, "connect network failed !!!");
    ...
    st_server_t * bootstrapServer)
{
    ...
    query_length = cis_utils_stringCopy(query, PRV_QUERY_BUFFER_LENGTH, "?ep=");
    if (query_length < 0)
    {
        bootstrapServer->status = STATE_BS_FAILED;
        return;
    }
    res = cis_utils_stringCopy(query + query_length, PRV_QUERY_BUFFER_LENGTH - query_length, context->endpointName);
    if (res < 0)
    {
        bootstrapServer->status = STATE_BS_FAILED;
        return;
