...
int FreeRTOS_write_rai(Network* n, unsigned char* buffer, int len, int timeout_ms, int rai, bool exceptdata)
{
    TickType_t xTicksToWait = timeout_ms / portTICK_PERIOD_MS; /* convert milliseconds to ticks */
    TimeOut_t xTimeOut;
    int sentLen = 0;
...
void NetworkInit(Network* n)
{
    ...
    n->mqttwrite = FreeRTOS_write_rai;
...
int NetworkSetConnTimeout(Network* n, int send_timeout, int recv_timeout)
{
  ...
  rx_timeout.tv_usec = (recv_timeout%1000)*1000;
    ...
    if(ret != 0)
    {
        ...
        DEBUG_TRACE(MQTT, P_INFO,"..11.mqtt socket set timeout fail..socket=%d.\r\n",n->my_socket);
        //return 1;
    }
    ret = FreeRTOS_setsockopt(n->my_socket, SOL_SOCKET, SO_RCVTIMEO, &rx_timeout, sizeof(rx_timeout));
    if(ret != 0)
    {
        DEBUG_TRACE(MQTT, P_INFO,"..22.mqtt socket set timeout fail...\r\n");
        //return 1;
    }
    INT32 flags = fcntl(n->my_socket, F_GETFL, 0);
    if(flags < 0)
    {
        DEBUG_TRACE(MQTT, P_INFO,"mqtt get file cntrl flags fail");
        return 1;
    }
