...
//add for RF TX spur issue, need to move the vector table from flash to ram
#define RAM_VECTOR_FOR_RF_SPUR
typedef void (*Ram_Vector)(void);
__attribute__((used)) static const Ram_Vector ramvector[25] __attribute__((__section__(".ramVector"))) = {0};
...
void SystemFullImageInit (void)
{
    ...
    #ifdef RAM_VECTOR_FOR_RF_SPUR
    memcpy((void*)ramvector,&__Vectors,sizeof(ramvector));
...
void SystemInit (void)
{
	...
	*(volatile uint32_t *)(0x4405000C) = *(volatile uint32_t *)(0x44000040);			 // copy wakeup edge setting to gpr
